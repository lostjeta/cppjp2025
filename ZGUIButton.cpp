...
ZGUIButton::ZGUIButton(ZGUIDialog* pDialog)
{
...
BOOL ZGUIButton::HandleKeyboard(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
BOOL ZGUIButton::HandleMouse(UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam)
{
    if (CanHaveFocus() == FALSE)
        return FALSE;
    ...
    case WM_LBUTTONUP:
    {
        if (m_bPressed)
        {
            ...
            if (ContainsPoint(pt))
            {
                m_pParentDialog->SendEvent(EVENT_BUTTON_CLICKED, this);
            }
            return TRUE;
        }
        break;
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (!m_bMouseOver &&
            ...
            ContainsPoint(pt))
        {
            m_pParentDialog->SendEvent(EVENT_BUTTON_OVER, this);
        }
        break;
    }
    };
...
void ZGUIButton::Render(ID3D11DeviceContext* pContext, float fElapsedTime)
{
    int nOffsetX = 0;
    int nOffsetY = 0;
...
void ZGUIButton::SetTextureColor(DirectX::XMFLOAT4 Color)
{
