...
namespace Bind
{
    class ZStencil : public ZBindable
    {
    public:
        enum class Mode
        {
        ...
        ZStencil(ZGraphics& gfx, Mode mode)
            :
            mode(mode)
        {
        ...
        void Bind(ZGraphics& gfx) noxnd override
        {
            INFOMAN_NOHR(gfx);
            GFX_THROW_INFO_ONLY(GetContext(gfx)->OMSetDepthStencilState(pStencil.Get(), 0xFF));
        }
        static std::shared_ptr<Stencil> Resolve(ZGraphics& gfx, Mode mode)
        {
