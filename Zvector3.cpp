...
float ZVector3::Dot(const ZVector3& u, const ZVector3& v) const {
...
//ZVector3::ZVector3(double x, double y, double z) : this->x;
...
double ZVector3::Dot(const ZVector3& rhs) const {
	return (this->x * rhs.x) + (this->y * rhs.y) + (this->z * rhs.z); // 자기자신x에 자기자신이 x이면 자기자신x의 제곱
}
...
double ZVector3::Length() const { // 내적
	// std::sqrt(x*x+y*y+z*z)
	return sqrt(Dot(*this));
}

double ZVector3::radBetween(const ZVector3& a, const ZVector3& b) const { // 내적을 라디안 값으로 변환하는 것
